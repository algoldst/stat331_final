...
server <- function(input, output) {
    ...
    output$text_explanation <- renderText("In 2015, the Washington Post published an investigation into police reporting in the US -- or rather, the lack of police reporting. As they note in their publication, no branch of US government is responsible for ensuring the collection or maintainance of proper reporting of lethal police shootings. While some departments have policies to record police shootings, many do not, and even the FBI's database of fatal shootings by police is horribly limited in its scope. Since then, the Post has maintained its own database of more than 5000 lethal shootings by police, collecting information on each individual shot and the details surrounding their death. However, this is all just data. While the Post has conducted its own analysis of the data, it's worth wondering about the validity of their investigation. We wanted to investigate some of the questions posed by the Washington Post, as well as pose some of our own, and therefore we have turned to their database in order to answer the questions we were wondering the most about."
    )
}
